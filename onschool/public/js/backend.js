...
$('#addcash').submit(function (e) { 
    e.preventDefault();
    
    var formdata  = new FormData(jQuery('#addcash')[0]),
     ...
     $.ajax({
         ...
         },error: function(xhr, exception)
         {
             ...
             if(exception == 'error')
             {  
                $('.wages-error').text(xhr.responseJSON.errors.wages);
                $('.purchases-error').text(xhr.responseJSON.errors.purchases);
                $('.sales-error').text(xhr.responseJSON.errors.sales);
                $('.pettycash-error').text(xhr.responseJSON.errors.pettycash); 
             }
 ...
 $(document).on("click",".editcash",function(){
    ...
    $.ajax({
        ...
        {
            $("#id").val(cash.id);
            $("#wages").val(cash.wages);
            $("#purchases").val(cash.purchases);
            $("#sales").val(cash.sales);
            $("#pettycash").val(cash.pettycash);
 ...
 $('#updatecash').submit(function(e){
    e.preventDefault();
    var formdata  = new FormData(jQuery('#updatecash')[0]) ,
        id  = $("#id").val(),
        url = $(this).data('url');
